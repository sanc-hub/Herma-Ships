...
const GOOGLE_CLIENT_ID = "224085854750-jbrd6k1ou0l11u728um38db0dlhkvkga.apps.googleusercontent.com";

// ── The root Drive folder where all vessel files are stored ──────
// This is the ROOT_FOLDER_ID already in your Apps Script backend
const DRIVE_ROOT_FOLDER_ID = "1-UJFyg-M6i9ydRxBVt54Bo3GKbFFjImW";
...
async function ensureDriveToken(forceConsent = false){
  ...
  if(!client) throw new Error('Google Identity Services failed to load');
  ...
  return new Promise((resolve, reject) => {
    ...
    const callback = response => {
      ...
      if(response.error){
        console.error('OAuth error:', response.error);
        if(/origin_mismatch|access_denied/i.test(response.error)){
          reject(new Error('DOMAIN_NOT_CONFIGURED'));
        } else {
    ...
    if(matchedFiles.length){
      ...
      searchFiles.innerHTML = matchedFiles.map(f=>{
        ...
        return `<div class="fleet-search-file" data-drive="${f.driveFileId||''}" data-excel="${f.excelDriveFileId||''}" data-name="${encodeHTML(f.name||'')}" data-vessel="${encodeHTML(vesselName)}" data-url="${f.driveFileId ? 'https://drive.google.com/file/d/'+f.driveFileId+'/preview' : ''}">
          ...
          <div class="fleet-search-file-actions"><button class="fleet-excel-btn" data-excel="${f.excelDriveFileId||''}">Excel</button></div>
        </div>`;
      }).join('');
      searchFiles.querySelectorAll('.fleet-search-file').forEach(el=>{
        ...
        const xbtn = el.querySelector('.fleet-excel-btn');
        if(xbtn){
          xbtn.onclick = (e)=>{
            e.stopPropagation();
            const excelId = xbtn.dataset.excel;
            if(excelId){
              openExcelApp(excelId);
            } else {
              toast('Excel not available for this document','warn');
            }
          };
        }
      });
...
async function handleFiles(fileList){
  ...
  failed.forEach(item => {
    ...
    if(errMsg === 'DOMAIN_NOT_CONFIGURED'){
      const origin = window.location.origin || 'unknown origin';
      toast('Domain not configured in Google Cloud. Authorize this origin: '+origin,'error');
    } else if(/popup|blocked|timed out/i.test(errMsg)){
      toast('Google auth popup blocked. Allow popups in browser settings.','warn');
    } else if(/access_denied|interaction_required|consent_required|origin_mismatch/i.test(errMsg)){
      const origin = window.location.origin || 'unknown origin';
      toast('Upload blocked by Google auth. Authorize origin: '+origin,'warn');
    } else {
...
function renderMain(){
  ...
  ${canEdit?`<div class="dz" id="dz">
    <div style="position:relative;pointer-events:none">
      ...
      <div class="dz-txt">Drag &amp; drop PDFs or Excel files here &nbsp;&#xb7;&nbsp; <em>click to browse</em></div>
      <div class="dz-hint">PDF / Excel only &#xb7; Metadata synced to all users instantly</div>
    </div>
  ...
  </div>`:`<div class="readonly-note"><i class="ti ti-eye"></i> User access: view and download documents only.</div>`}
...
function renderFilesList(shown){
  ...
  shown.forEach(file=>{
    html += `<tr class="fr" data-key="${file.key}" data-drive="${file.driveFileId||''}" data-excel="${file.excelDriveFileId||''}">
      ...
      <td class="td-xls"><button class="xlsbtn" data-excel="${file.excelDriveFileId||''}" title="Open Excel">Excel</button></td>
  ...
  el.querySelectorAll('.xlsbtn').forEach(btn=>{
    btn.onclick = e=>{
      e.stopPropagation();
      const excelId = btn.dataset.excel || btn.closest('tr')?.dataset.excel;
      if(!excelId){ toast('Excel not available for this document','warn'); return; }
      openExcelApp(excelId);
    };
  });
}
